...
if ($htaccess_file_editor_restore_result === false) {
    ...
    echo '<p>' . wp_kses_post( __('The backup file is located in the <code>wp-content</code> folder.', 'htaccess-file-editor') ) . '</p>';
    echo '</div>';
} else {
    ...
    echo '<textarea id="htaccess-file-editor-textarea" class="htaccess-file-editor-textarea">' . $htaccess_file_editor_restore_result . '</textarea>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
    echo '</div>';
}